...
Mat GaussianNxNFilterColor(const Mat& image, int N) {
    Mat blurredImage(image.size(), CV_8UC3);

    vector<vector<int>> pascal(N, vector<int>(N, 0));
    vector<vector<float>> kernel(N, vector<float>(N, 0.0f));
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            kernel[i][j] = pascal[N - 1][i] * pascal[N - 1][j];
    ...
    if (N % 2 == 0) {
        for (int i = A; i < image.rows - A; ++i) {
            for (int j = A; j < image.cols - A; ++j) {
                ...
                for (int m = -A; m <= A - 1; ++m) {
                    for (int n = -A; n <= A - 1; ++n) {
                        Vec3b intensity = image.at<Vec3b>(i + m, j + n);
                        sum[0] += intensity[0] * kernel[m + A][n + A];
                        sum[1] += intensity[1] * kernel[m + A][n + A];
                        sum[2] += intensity[2] * kernel[m + A][n + A];
                    }
                }
                blurredImage.at<Vec3b>(i, j) = Vec3b(sum[0] / kernelSum, sum[1] / kernelSum, sum[2] / kernelSum);
            }
        }
    }
    else {
...
Mat GaussianNxNFilterGray(const Mat& image, int N) {
    Mat blurredImage(image.size(), CV_8UC1);
    ...
    if (N % 2 == 0) {
        for (int i = A; i < image.rows - A; ++i) {
            for (int j = A; j < image.cols - A; ++j) {
                ...
                for (int m = -A; m <= A - 1; ++m) {
                    for (int n = -A; n <= A - 1; ++n) {
                        sum += kernel[m + A][n + A] * image.at<uchar>(i + m, j + n);
                    }
                }
                blurredImage.at<uchar>(i, j) = sum / kernelSum;
            }
        }
    }
    else {
